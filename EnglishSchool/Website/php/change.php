    ...
    global $db_data;
        ...
        while($value = $stmt->fetch()){
          ...
          $db_data .= "<tr><td>$value[id]</td><td>$value[name]</td><td>$value[kana]</td><td>$value[birthday]</td><td>$gender</td>
            ...
            <form action='!option!.php' method='post' enctype='multipart/form-data'><input type='submit' name='send' value='変更'></form>";
        }
        $db_data .= "</table>";
			}
...
function show_db(){
  ...
  fclose($conf);
  session_start();
  $data = str_replace("!function!", "変更", $data);
  $data = str_replace("!option!", "change", $data);
